...
void InitCommandLine(int argc, char** argv, po::variables_map* conf) {
  ...
  opts.add_options()(
      "reference,r", po::value<vector<string> >(),
      "[1 or more required] Reference translation(s) in tokenized text files")(
      "evaluation_metric,m", po::value<string>()->default_value("IBM_BLEU"),
      "Evaluation metric (ibm_bleu, koehn_bleu, nist_bleu, ter, meteor, etc.)")(
  ...
  if (!conf->count("reference")) {
    cerr << "Please specify one or more references using -r <REF1.TXT> -r "
            "<REF2.TXT> ...\n";
    flag = true;
  }
...
int main(int argc, char** argv) {
  ...
  string loss_function =
      UppercaseString(conf["evaluation_metric"].as<string>());
  if (loss_function == "COMBI") {
    cerr << "WARNING: 'combi' metric is no longer supported, switching to "
            "'COMB:TER=-0.5;IBM_BLEU=0.5'\n";
  ...
  cerr << "Loaded " << ds.size() << " references for scoring with "
       << loss_function << endl;
  ...
  CHECK_GT(lc, 0);
  if (lc > ds.size()) {
    cerr << "Too many (" << lc << ") translations in input, expected "
         << ds.size() << endl;
    return 1;
  }
  if (lc != ds.size())
    cerr << "Fewer sentences in hyp (" << lc << ") than refs (" << ds.size()
         << "): scoring partial set!\n";
